...
from qiskit_experiments.measurement.discriminator import (
    ...
    DiscriminatorAnalysis, MixedDiscriminatorAnalysis, MixedDiscriminatorExperiment
)
...
class TestDiscriminator(QiskitTestCase):
    ...
    def test_single_qubit_mixed(self):
        backend = DiscriminatorBackend()
        exp = MixedDiscriminatorExperiment(1)
        res = exp.run(backend, shots=10, meas_level=1,
        meas_return="single", discriminator_type='GM', distance=[[0.5,0.5,0.5,0.5]]).analysis_result(0)
