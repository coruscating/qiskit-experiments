"""
Methods used for mixed states discrimination.
"""
...
class GeometricMethod():
    ...
    def __init__(self,  distance = None, discriminator_type = 'EM'):
        """ This technique classify the data outside the distance from each centroids as mixed state

        Args:
            distance (list): Optional, the coordinates in IQ space to each cluster 0 and 1 eg:
            [0.5, 0.5, 0.5, 0.5]
            discriminator_type (str): the type of discriminator or bounds around the centroids,
            EM for Ellipsoids and RM for Rectangles 
        """
        self._means = []
        self._std = []
        ...
        if distance is None:
            ...
            if all(isinstance(x, float) for x in distance):
                self.distance0 = [distance[0], distance[1]]
                self.distance1 = [distance[2], distance[3]]
            else:
                raise ValueError(' The values in distance must be float')
        else:
            raise ValueError(' distance must be a list or array of float of length 4')
    ...
    def predict(self, y_data, return_proba = False):
        ...
        if self.distance0 is None:
            self.distance0 = (self._means[0] - self._means[1])/2
            self.distance1 = (self._means[0] - self._means[1])/2
        for value in y_data:
            a = value[0]
            b = value[1]
...
class GeometricMethodSklearn(GeometricMethod):
    """ LDA and QDA models for mixed states classification

        Args:
            threshold (list): Optional, the threshold of probability from which the estimation
            is uncertain for cluster 0 and 1 eg: [0.1, 0.1]. The values are less than 0.5
            discriminator_type (str): the type of discriminator LDA or QDA
    """
    def __init__(self,  threshold = None, discriminator_type = 'LDA'):
        ...
        if threshold is None:
            ...
            if all(isinstance(x, float) and x < 0.5 for x in threshold):
                self._threshold = threshold
            else:
                raise ValueError(' The values in threshold must be float')
        else:
            raise ValueError(' threshold must be a list or array of float of length 2')
...
class GaussianMixtureModel():
    def __init__(self,  threshold = None):
        """ Estimate the distribution of mixed states using a gaussian mixture model
        ...
        # store the gaussian estimators with the greatest weigtht for each data calibration 0 and 1
        self.gaussian01 = []
